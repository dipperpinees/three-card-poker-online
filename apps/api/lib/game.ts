...
export default class Game {
    ...
    sendCashMessage(socketId: string, type: string | undefined, cash: number) {
        const message = type === 'win' ? `You have received ${cash}$` : `You have lost ${cash}$`;
    ...
    addPlayer(socket: socketio.Socket, name: string, pos: number, cash: number, avatar: string) {
        if (this.isPlayed) {
            socket.emit('alert', { message: 'Please wait for the game to end' });
            return;
        }
    ...
    changeMaster(socket: socketio.Socket, newMasterId: string) {
        ...
        if (socket.id === this.master?.socketId) {
            ...
            socket.broadcast.emit('alert', {
                message: `${this.mapPlayer[newMasterId].name} is transferred to the role of dealer`,
            });
        }
    }
    ...
    sendCash(socket: socketio.Socket, cash: number) {
        ...
        if (Number(cash) + this.mapPlayer?.[socket.id]?.cashOther > this.maxCash) {
            socket.emit('alert', { message: 'The amount exceeds the limit' });
            return;
        }
    ...
    putOther(socket: socketio.Socket, putId: string, putCash: number) {
        ...
        if (putCash < 1000) {
            socket.emit('alert', { message: 'Error! Minimum amount 1000$' });
            return;
        }
        ...
        if (
            ...
            socket.emit('alert', {
                message: 'The amount of people placed is over the limit',
            });
            return;
        }

        if (this.isPlayed) {
            socket.emit('alert', { message: 'Error! The game has been started' });
            return;
        }

        if (this.mapPlayer[socket.id].putOther.length > 1) {
            socket.emit('alert', { message: 'Can only bet with a maximum of 2 players' });
            return;
        }

        this.mapPlayer[putId].cashOther += putCash;
        this.mapPlayer[socket.id].putOther.push({ putId, putCash });
        socket.emit('alert', { message: `Bet successfully` });
        this.io.to(putId).emit('alert', {
            message: `${this.mapPlayer[socket.id].name} asked me to place ${putCash}$`,
        });
        this.update();
    }
    ...
    sendOther(socket: socketio.Socket, recipientId: string, recipientCash: number) {
        ...
        this.io.to(recipientId).emit('alert', {
            type: 'win',
            message: `You get ${recipientCash}$ from  ${this.mapPlayer[socket.id].name}`,
        });
        socket.emit('alert', { message: 'Cash transfer successful' });
        this.update();
    }
    ...
    contributeCash(socket: socketio.Socket) {
        if (!this.statusPlayer(socket.id)) return;
        if (this.mapPlayer[socket.id].contribute) {
            socket.emit('alert', { message: 'Failed to contribute' });
            return;
        }
        ...
        socket.emit('alert', { message: 'Contribute successfully' });
        this.update();
        this.updateContribute();
    }
    ...
    compareContribute() {
        ...
        this.io.to(winPlayer.socketId).emit('alert', {
            message: `You get ${players.length * this.contribute.cash}$ (contribute)`,
            type: 'win',
        });
    }
