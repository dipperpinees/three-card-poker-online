...
function Board({ onShowJoinForm }: Props) {
    ...
    const handleShowForm = (recipientId: string, name: string, type: string) => {
        if (type === 'send') {
            ...
            setMessage('The amount of money you want to give to ' + name);
            setFormType('send');
            setShowForm(true);
        } else {
            setSendId(recipientId);
            setMessage('The amount of cash you want to bet ' + name);
            setFormType('put');
            setShowForm(true);
        }
    };
    ...
    return (
        <div className="board">
            ...
            {listPlayer.map((player, index) => (
                <Player
                    isYou={socket.id === player.socketId}
                    key={index}
                    player={player}
            ...
            {showForm && (
                <SendCash
                    ...
                    onSubmit={formType === 'send' ? handleSendCash : handlePutCash}
                    title={message}
                />
            )}
        </div>
